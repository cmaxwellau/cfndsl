...
module CfnDsl
  ...
  class OrchestrationTemplate < JSONable
    ...
    def validate_rules(errors = [])
      ...
      rules.each_pair do |logical_id, rule|
        path = "/Rules/#{logical_id}"

        unless !rule.condition || conditions.include?(rule.condition)
          errors << "Invalid Condition : Rule #{logical_id} refers to missing Condition #{rule.condition}"
        end
